...
select name,city from guest group by city,name order by city;
...
select room_no,avg(price) as "average price" from room group by room_no;

//8. List hotel names, their room numbers, and the type of that room.

select hotel.name,room.room_no,room.type from hotel inner join room on hotel.hotel_no=room.hotel_no;
...
select count(date_from) as "Bookings from Sept"  from booking where extract(month from date_from)=09;
